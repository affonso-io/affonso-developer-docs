...
| `showDescription` | `true`, `false` | Show the affiliate program description as its own card below the header. Default: `false` |
...
| `showPayouts` | `true`, `false` | Show the Payout tab (recent payouts plus payout-method setup). Default: `false`. Omit to keep the tab hidden. |
...
**Example with light mode, Japanese, and the optional description card enabled:**
```html
<iframe
  src="https://affonso.io/embed/referrals?token=emb_...&theme=light&lang=ja&showDescription=true"
  ...
/>
```
...
<Frame>
  <img src="/images/embed-preview-editor.png" alt="Embed preview editor in Affonso dashboard" />
</Frame>
