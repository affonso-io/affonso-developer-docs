...
api: "POST https://api.affonso.io/v1/affiliates"
description: "Create a new affiliate Account and program contact"
---
...
<ParamField body="email" type="string" required>
  The affiliate's email address. Must be a valid email format. Creates or resolves an affiliate Account and a durable program contact for this email. Does not create a merchant User.
</ParamField>
...
<ParamField body="status" type="string" default="approved">
  Partnership status at create time. No follow-up update call is required. Valid values: `pending`, `approved`, `rejected`. Omit to keep the existing auto-approve behavior (`approved`).
</ParamField>
...
<ResponseField name="partnership_status" type="string">
  Partnership status with the program. Uppercase: `"PENDING"`, `"APPROVED"`, `"REJECTED"`, or `null`. Matches the `status` you sent. Defaults to `"APPROVED"` when `status` is omitted.
</ResponseField>
...
- Creates an affiliate Account and durable program contact. Existing program IDs, tracking, and financial relations stay intact. Does not create a customer/merchant User.
- Set `status` on create when the affiliate should start as `pending` or `rejected`. You do not need a second `PATCH` to change status after create.
