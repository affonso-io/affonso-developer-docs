...
Use this endpoint to track when a visitor clicks an affiliate link. This is useful for server-side tracking implementations. The created click record can later be referenced by its ID as `click_id` in Affonso server-side tracking endpoints.
...
<ResponseField name="id" type="string">
  Unique identifier for the click. Internally this is the `referral.id` of the click record. You can use it as `click_id` in endpoints such as `POST /v1/events`, `POST /v1/conversions`, or the Segment adapter (`properties.click_id`).
</ResponseField>
...
**Note:** If this click is the first stable identifier you have, keep the returned `id` and send it as `click_id` in your first backend conversion or event request. Once the referral is linked to your own `customer_id` or `external_user_id`, later events can use those identifiers instead.
