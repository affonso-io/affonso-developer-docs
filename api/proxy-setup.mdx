...
`CF-Connecting-IP`, `True-Client-IP`, or `X-Real-IP`. If your proxy or CDN
replaces the visitor IP with its own edge or internal IP, attribution can still
work, but country reporting can be wrong or missing.
...
Use that guide in particular if:

- clicks are tracked but countries are wrong or missing
- you use Cloudflare, NGINX, or another proxy in front of Affonso
- you need to verify which client-IP headers are actually reaching Affonso
