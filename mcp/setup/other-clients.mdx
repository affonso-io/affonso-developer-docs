...
Affonso MCP uses the **Streamable HTTP** transport, which is the standard for cloud-hosted MCP servers. Any client that supports this transport can connect to Affonso. The server negotiates protocol `2026-07-28` and still accepts legacy initialize-based clients on the same URL.
...
For custom implementations, MCP requests are standard HTTP POST. Send `Accept: application/json, text/event-stream`. A missing Accept header returns HTTP 406. An unsupported protocol version returns HTTP 400.
