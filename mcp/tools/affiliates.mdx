...
<ParamField body="status" type="string" default="approved">
  Initial partnership status: `pending`, `approved`, or `rejected`. Omit to auto-approve. No second update call is required.
</ParamField>
...
<ParamField body="invoice_details" type="object">
  Partial billing and invoice details. Omitted keys keep their current values. `null` clears optional fields. Creating a first invoice record requires `first_name`, `last_name`, `address_line_1`, `city`, and `country`.
</ParamField>

<ParamField body="payout_method" type="string">
  Payout method (`none` to remove). Valid values match `PUT /v1/affiliates/{id}`.
...
<ParamField body="payout_details" type="object">
  Payout details for the selected method. Sending this object replaces credentials and clears fields from other methods. Omit it to keep stored credentials when you only change `payout_method`.
</ParamField>
...
Set affiliate aff_abc123 payout method to PayPal without changing the saved email
```
