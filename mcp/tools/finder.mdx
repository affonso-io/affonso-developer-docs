...
The Finder tools start discovery searches, list results, and manage the team shortlist. All tools support `response_format` (`markdown` or `json`). `responseFormat` is accepted as an alias. Markdown returns the same JSON payload inside a fenced `json` block. List tools use `limit` and `offset`, not `page`.
